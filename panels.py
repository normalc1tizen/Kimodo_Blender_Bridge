...
class KIMODO_PT_Help(KIMODO_PanelBase, Panel):
    ...
    def draw(self, context):
        ...
        col.separator()
        col.label(text="1. Install Kimodo (pip install -e kimodo-src)")
        col.label(text="2. Set Python path → Start Kimodo")
        col.label(text="   (model loads once, stays hot)")
        col.separator()
        col.label(text="3. Enter prompt → Generate Motion")
        col.label(text="4. Pick source + target rigs")
        col.label(text="5. Auto-Match → Apply Constraints")
        col.label(text="6. Bake when satisfied")
        col.separator()
        col.label(text="Low VRAM? Run separately:", icon='INFO')
